...
def solve(d, no_elves_die):
    input_data = open("../data/day15/input.txt").read().strip()
    ...
    while True:
        ...
        for idx, c in enumerate(order):
            ...
            if not enemy_positions_in_range:
                ...
                if closest_targets:
                    ...
                    for s in sorted(nearby_cells, key=reading_order):
                        ...
                        if d == dist - 1:
                            # print(f"Move {c.unit_type} ({c.x}, {c.y}) -> {s} {choice}")
                            c.x, c.y = s
                            break
                else:
                    # print(f"Noop {c.unit_type} ({c.x}, {c.y})")
                    pass
            ...
            if enemy_positions_in_range:
                ...
                # print(f"Attack {c.unit_type} ({c.x}, {c.y}) -> ({lowest_health.x}, {lowest_health.y})")
